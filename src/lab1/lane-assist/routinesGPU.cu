...
__global__ void canny_kernel(uint8_t *im, uint8_t *image_out,
    ...
    int height, int width)
{
    ...
    uint8_t *d_im, *d_image_out, *d_pedge;
    ...
    // Launch kernel
    canny_kernel<<<gridSize, blockSize>>>(d_im, d_image_out, d_NR, d_G, d_phi, d_Gx, d_Gy, d_pedge, level, height, width);
